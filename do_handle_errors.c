...
int do_handle_errors(Shell_pack *sh_data, int return_val)
{
	...
	else if (return_val == 126)
		err_permission(sh_data);
...
void err_for_environ(Shell_pack *sh_data)
{
	...
	_len = makeshift_strlen(sh_data->sh_fname) + makeshift_strlen(count_str)
	...
	makeshift_strcpy(full_error_msg, sh_data->sh_fname);
	makeshift_strcat(full_error_msg, ": ");
	makeshift_strcat(full_error_msg, count_str);
	makeshift_strcat(full_error_msg, ": ");
	makeshift_strcat(full_error_msg, sh_data->sh_arguments[0]);
	makeshift_strcat(full_error_msg, e_msg);
	makeshift_strcat(full_error_msg, "\0");
	free(count_str);
...
void err_for_exit(Shell_pack *sh_data)
{
	...
	makeshift_strcat(full_error_msg, sh_data->sh_arguments[0]);
	makeshift_strcat(full_error_msg, e_msg);
	makeshift_strcat(full_error_msg, sh_data->sh_arguments[1]);
	makeshift_strcat(full_error_msg, "\n\0");
	free(count_str);
...
/**
 * err_permission - displays error message for command not found
 * @sh_data: Data from Shell_pack
 */
